...
function bookingMovie(state = initialState, action) {
  switch (action.type) {
    case "booking/add_del_seats": {
      const isExist = state.bookingSeats.find((item) => {
        return item.soGhe === action.payload.soGhe;
      });
      if (isExist) {
        const newBookingSeats = state.bookingSeats.filter((item) => {
          return item.soGhe !== action.payload.soGhe;
        });
        return {
          ...state,
      ...
      const newBookingSeats = [...state.bookingSeats, action.payload];
      return {
        ...state,
        bookingSeats: newBookingSeats,
      };
